...
def create_interview(deal_id: str, founder_email: str, founder_name: str = None) -> Dict[str, Any]:
    """Create new interview session and store in deal document"""
    token = generate_interview_token()
    
    # Get deal data to identify issues
    ...
    # Check if interview already exists and is active
    if 'interview' in deal_data and deal_data['interview'].get('status') in ['pending', 'active']:
        raise ValueError("Interview already in progress for this deal")
    
    # Identify issues (missing, shallow, needs_detail)
    issues = identify_missing_and_shallow_fields(deal_data)
    
    # Flatten all issues into a single list with priority
    all_issues = []
    
    # Priority order: missing (critical/high) -> shallow (critical/high) -> needs_detail -> lower priority
    ...
    if not all_issues:
        raise ValueError("No missing or incomplete information found. Interview not needed.")
    
    interview_data = {
        "status": "pending",
        "token": token,
        "founder_email": founder_email,
        "founder_name": founder_name or "Founder",
        "created_at": datetime.utcnow().isoformat() + "Z",
        "started_at": None,
        "completed_at": None,
        "expires_at": (datetime.utcnow() + timedelta(days=7)).isoformat() + "Z",
        "issues": all_issues,  # All issues with context
        "missing_fields": [issue['field'] for issue in all_issues],  # Just field names for compatibility
        "chat_history": [],
        "gathered_info": {}
    }
    
    # Store interview data in deal document
    deal_ref.update({
    ...
    return {
        ...
        "total_questions": len(all_issues),
        "critical_questions": len([i for i in all_issues if i['importance'] == 'critical']),
        "breakdown": {
            "missing": len(issues.get('missing', [])),
            "shallow": len(issues.get('shallow', [])),
            "needs_detail": len(issues.get('needs_detail', []))
        }
    }
