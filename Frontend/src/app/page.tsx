...
export default function InvestorDashboard() {
  ...
  const fetchDeals = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/deals`);
      if (!response.ok) {
  ...
  const handleDelete = async (startupId: string) => {
    ...
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/deals/${startupId}`, {
            method: 'DELETE',
        });
        if (!response.ok) {
  ...
  const handleDownload = async (dealId: string, companyName: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/download_memo/${dealId}`);
      if (!response.ok) {
